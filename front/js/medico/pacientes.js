/**
 * medico/pacientes.js — Pacientes for the logged-in Medico
 */
...
async function loadData() {
  tbody.innerHTML = '<tr class="loading-row"><td colspan="6"><div class="spinner-border spinner-border-sm text-primary me-2"></div>Cargando…</td></tr>';
  try {
    pacientes = await apiGet('/pacientes');
    renderTable();
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center text-danger py-3"><i class="bi bi-exclamation-triangle me-2"></i>${escapeHtml(err.message)}</td></tr>`;
  }
}
...
function renderTable() {
  tbody.innerHTML = '';
  const myPacientes = Array.isArray(pacientes) ? pacientes : [];

  if (myPacientes.length === 0) {
    const tr = document.createElement('tr');
    const td = document.createElement('td');
    td.colSpan = 6;
    td.className = 'text-center py-4 text-muted';
    td.textContent = 'No tiene pacientes registrados.';
    tr.appendChild(td);
